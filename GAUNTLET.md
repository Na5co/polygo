...
- 2026-09-17 · post-gauntlet "fix all" · (1) plural translation for .xcstrings variations + substitutions, Android <plurals>, gettext msgid_plural — one unit per CLDR category per target locale, concrete-count hints; live qwen3:8b: 8/8 pl, 6/8 ru forms correct; IceCubes pl dry-run plans exactly the 32 missing forms. (2) `fragment` check + provider repair/quarantine for source words left in non-Latin-script translations; 5 hits on DDG+IceCubes, all defensible. (3) `polygo doctor`. (4) README/docs/CHANGELOG/launch updated, GIF re-recorded with doctor; rubrics gemma4 5/5 and 7/7. 82 tests. CI matrix still unverified until the repo exists on GitHub.
