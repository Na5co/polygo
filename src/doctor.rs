...
pub fn run(root: &Path) -> Vec<Check> {
    ...
    match cfg.provider.kind.as_str() {
        ...
        "openai" | "anthropic" => {
            ...
            let has_key = std::env::var("POLYGO_API_KEY").is_ok()
                || std::env::var(key_var).is_ok()
                || crate::models::stored_api_key(kind).is_some();
            if has_key || (kind == "openai" && cfg.provider.base_url.is_some()) {
                ...
                out.push(fail(
                    ...
                    format!("polygo use {kind}/{model} --api-key …   (or export {key_var})"),
                ));
            }
        }
        other => out.push(fail(
...
fn check_ollama(cfg: &Config, model: &str) -> Vec<Check> {
    ...
    match tags {
        Err(e) => vec![fail(
            ...
            "install from https://ollama.com (macOS: brew install ollama), run `ollama serve`, or `polygo use openai/gpt-4o-mini --api-key …`",
        )],
        Ok(v) => {
            ...
            if names.iter().any(|n| n == &want || n == model) {
                ...
                out.push(fail(
                    ...
                    format!("polygo use {model}   (pulls it; `polygo models` lists options)"),
                ));
            }
            out
        }
