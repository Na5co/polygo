...
pub struct Report {
    ...
    /// Strings filled from the translation memory without a model call.
    pub reused: usize,
}
...
pub fn translate(
    ...
    // Translation memory: learn what humans wrote here, reuse what humans wrote elsewhere.
    // The mock provider is the test double: keep it away from the user's real memory
    // file unless a test points POLYGO_CONFIG_DIR somewhere of its own.
    let use_memory =
        cfg.memory && (provider.name() != "mock" || std::env::var("POLYGO_CONFIG_DIR").is_ok());
    let mut memory = if use_memory {
        Some(crate::memory::Memory::load())
    } else {
        None
    };
        ...
        for locale in &locales {
            ...
            for key in report.planned.get(locale).cloned().unwrap_or_default() {
                ...
                if let Some(t) = mem.recall(locale, &u.source) {
                    ...
                    if opts.verbose {
                        eprintln!("  {locale}  {key}  →  {t}  (translation memory)");
                    }
                    reused.push(key);
                }
            }
                ...
                (
                    ...
                    keys.iter()
                        .map(|k| {
                            ...
                            if opts.context {
                                let mut candidates: Vec<(String, String)> = units
                                    .iter()
                                    .filter_map(|o| {
                                        o.translations
                                            .get(locale)
                                    ...
                                    .collect();
                                if let Some(mem) = &memory {
                                    candidates.extend(mem.examples(locale));
                                }
                                let examples =
        ...
        std::thread::scope(|s| {
            ...
            for _ in 0..total {
                ...
                for t in outcome.translations {
                    ...
                    if let Some(mem) = &mut memory {
                        mem.learn(locale, &u.source, &t.text, provider.model());
                    }
                    if opts.verbose {
...
pub fn write_translation(
    ...
    write_translations(root, cfg, &[(key, locale, text)])
}

/// Write several `(key, locale, text)` at once: files are parsed and flushed once.
pub fn write_translations(root: &Path, cfg: &Config, items: &[(&str, &str, &str)]) -> Result<()> {
    let mut ws = Workspace::open(root, cfg)?;
    for (key, locale, text) in items {
