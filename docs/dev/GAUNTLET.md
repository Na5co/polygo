...
- [x] G0.1 Script `scripts/killtest.sh`: takes one `.xcstrings` + target locale, runs Qwen3-8B via Ollama twice — (a) key + source string only, (b) with code-usage context + 3 similar existing translations — writes `bench/killtest/<locale>.csv` with both outputs side by side, 30 rows, shuffled, blinded. Then `scripts/killtest.py --judge bench/killtest/<locale>.csv --judge-model gemma4` has a different model pick A/B/tie per row (blind to which is which) and `--score` applies the kill rule.
  - Acceptance: `python3 scripts/killtest.py --judge bench/killtest/<locale>.csv --judge-model gemma4 && python3 scripts/killtest.py --score bench/killtest/<locale>.csv` prints PASS on at least one of two locales: one without existing translations (code context only) and one with (e.g. `de`, so few-shots also apply).
...
- 2026-09-17 · post-gauntlet "fix all" · (1) plural translation for .xcstrings variations + substitutions, Android <plurals>, gettext msgid_plural — one unit per CLDR category per target locale, concrete-count hints; live qwen3:8b: 8/8 pl, 6/8 ru forms correct; IceCubes pl dry-run plans exactly the 32 missing forms. (2) `fragment` check + provider repair/quarantine for source words left in non-Latin-script translations; 5 hits on DDG+IceCubes, all defensible. (3) `polygo doctor`. (4) README/docs/CHANGELOG/launch updated, GIF re-recorded with doctor; rubrics gemma4 5/5 and 7/7. 82 tests. Repo pushed to github.com/Na5co/polygo (owner corrected from atanasa); v0.1.0-rc.2 release matrix green on all 5 targets (assets 2.0–2.5 MB, SHA256SUMS), ci green on ubuntu/macos/windows after four Windows-only fixes (musl via cross, backslash paths in init + prompts, unix-gated shell tests, disk-bound perf budget). install.sh → doctor works on this Mac; aarch64 static binary runs in Alpine.
- 2026-09-17 · user test on Loop (real repo, +bg) · two real bugs found and fixed: (1) `init` mistook packages/{cli,mcp}/tsconfig.json for i18next locales → language-code list + string-only JSON check; (2) placeholder mismatch (`%1$@ … %@ (%@)` from gemma4) was only caught by `check`, not the translate-time repair loop → validate() now runs placeholders::compare (repair, then quarantine). New: `polygo models` + `polygo use <model>` (Ollama pull with progress, API providers, --api-key stored 0600, --global default for init). qwen3:8b Bulgarian judged rough by a native speaker; gemma4 clearly better → README recommends `polygo use gemma4` for smaller languages. 88 tests.
- 2026-09-17 · "micro wins" batch · `polygo audit` (second-model grading; on Loop bg gemma4 caught qwen3 turning "above" into "по-надолу" — a meaning error `check` cannot see; judges answer with source text instead of keys → tolerant matching), translation memory across projects (human entries reused verbatim, model entries as examples; mock kept out of the real file), `polygo pseudo` (en-XA, placeholders preserved, 403 Loop strings, check ok), `polygo status --markdown` (+ PR body in the Action), comment directives `polygo:skip` / `polygo:max=N` (enforced in check and the repair loop). 95 tests.
