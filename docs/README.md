...
`[keys] ignore = { "legal.*" = ["length"], "brand" = ["identical"] }` keeps `check` quiet about those codes for matching keys, and only those; `polygo:ignore=identical,length` in a developer comment does the same for one key.
...
| `polygo check [PATH] [--locale L] [--json] [--github] [--strict] [--all] [--write-baseline] [--no-baseline] [--fix]` | placeholders, plurals, lengths, untranslated fragments. `PATH` (a string file or a directory) needs no `polygo.toml`: format and locales are detected. `--fix` re-translates the failures (configured project only) |
