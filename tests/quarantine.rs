...
#[test]
fn broken_placeholder_is_repaired_then_quarantined() {
    // Seen live: gemma4 wrote `%1$@ … %@ (%@)` for a source with three `%@`; only
    // `polygo check` caught it afterwards. Now the repair loop does.
    let src = "{\n  \"k1\": \"First string\",\n  \"k2\": \"Delete %@ from %@?\",\n  \"k3\": \"Third string\"\n}\n";
    let dir = tempfile::tempdir().unwrap();
    project(dir.path());
    ...
    let de = fs::read_to_string(dir.path().join("locales/de.json")).unwrap();
    assert!(de.contains("\"k2\": \"⟦de⟧ Delete %@ from %@?\""), "{de}");
    ...
    let err = String::from_utf8_lossy(&out.stderr);
    assert!(
        err.contains("k2") && err.contains("placeholders changed"),
        "{err}"
    );
    let de = fs::read_to_string(dir.path().join("locales/de.json")).unwrap();
    assert!(
        !de.contains("\"k2\""),
        "broken translation must not be written:\n{de}"
    );
}
