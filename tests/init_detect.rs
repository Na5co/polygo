...
#[test]
fn init_ignores_monorepo_packages_and_non_locale_json() {
    // packages/cli/tsconfig.json + packages/mcp/tsconfig.json looked like i18next
    // namespaces for the "languages" cli and mcp (found on a real monorepo).
    let dir = tempfile::tempdir().unwrap();
    let root = dir.path();
    for pkg in ["cli", "mcp"] {
        fs::create_dir_all(root.join("packages").join(pkg)).unwrap();
        fs::write(
            root.join("packages").join(pkg).join("tsconfig.json"),
            "{\n  \"compilerOptions\": { \"strict\": true, \"target\": \"es2022\" }\n}\n",
        )
        .unwrap();
    }
    // A config dir with `en.json` full of non-string leaves is not a locale either.
    fs::create_dir_all(root.join("locales")).unwrap();
    fs::write(root.join("locales/en.json"), "{\n  \"enabled\": true\n}\n").unwrap();
    let err = polygo::init::detect(root).unwrap_err().to_string();
    assert!(err.contains("no localization files"), "{err}");
